...
abstract class AbstractPackageAction extends AbstractAction
{
    ...
    protected function downloadFile($packageName, $packageVersion)
    {
        ...
        $fileVersionList->getConditionBuilder()->add('packageName = ?', [$packageName]);
        $fileVersionList->getConditionBuilder()->add('packageVersion = ?', [$packageVersion]);
        ...
        if (!$fileVersion->canDownload() && !(isset($_SERVER['PHP_AUTH_USER']) && isset($_SERVER["PHP_AUTH_PW"]))) {
            ...
            try {
                ...
                if ($e->getField() == 'username') {
                    try {
                        $user = EmailUserAuthentication::getInstance()->loginManually(
                            $_SERVER['PHP_AUTH_USER'],
                            $_SERVER["PHP_AUTH_PW"]
                        );
